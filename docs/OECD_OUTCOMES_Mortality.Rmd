---
title: "OECD Life Expectancy and Infant Mortality"
...
## Introduction
This notebook creates the plots for Lecture-01 - Course Introduction for my sections of SOC 1120.
...
## Load Data
All of the raw data for these slides were obtained from the OECD's open data website:
...
## Clean Data
First, we'll clean the health care spending data by obtaining total spending and retaining only the value for the latest year available:
...
For infant mortality, some countries do not have data for 2017 or 2018 so we need to go back further to get the latest year available:
...
Finally, we'll get the latest available data for life expectancy at birth. We only want total life expectancy, which pools men and women together. As with the other tables, we'll keep it only for the latest year available:
...
## Combine Tables
Next, we'll combine all four of our raw data tables into a single object:
...
### Infant Mortality
Finally, we'll generate a bar plot of countries based on their infant mortality rates:
